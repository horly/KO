    ...
    .container-tab1 {
      ...
      .form_card
       ...
       #libsearchcatart {
         ...
         <title>KEDIS Online! | Paiement note de crédit client</title>
    <!--c head-->
    </head>
    <!--*****************************************************-->
    <body class="bg-light">
                            ...
                            <div class="page-header float-right">
                                <div class="page-title">
                                    <ol class="breadcrumb text-right">
                                        ...
                                        <li class="active">Paiement client</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card">
                  <h6 class="card-header">
                     <ul class="nav nav-pills card-header-pills">
                      <li class="nav-item">
                        <a class="nav-link" href="paiement_client.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>&id_cmp=<?php echo $id_cmp; ?>"><strong>Factures</strong></a>
                      </li>
                      <li class="nav-item">
                  ...
                  <?php 
                      ...
                      $view = $bdd->prepare("SELECT * FROM note_credit WHERE id_UE_cr = ? AND reste_a_payer_cr != 0 AND complete_credit != ?");
                    ...
                    ?>
                      ...
                      <div class="card-body">
                          <div class="card border-info mb-3" >
                              <div class="card-header text-left"><strong><b>iste des notes de crédit à rembourser (Total : <?php echo $existallvente; ?>)</b></strong></div>
                                <div class="card-body">
                                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    ...
                                    <tbody class="table-bordered" id="table_article">
                                      <?php
                                            ...
                                            $get_note = $bdd->prepare('SELECT * FROM note_credit WHERE id_UE_cr = ? AND reste_a_payer_cr != 0 ORDER BY id_cr DESC');
                                         ...
                                         ?>   
                                            <tr>
                                              ...
                                              <td class="text-left"><?php echo $row1['date_cr']; ?></td>
                                              <td class="text-left"><?php echo $row1['echeance_cr']; ?></td>
                                              <td class="text-left">
                                                <?php
                                                    //on recupère le fournisseur qui est le debiteur
                                                    $get_debiteur = $bdd->prepare('SELECT * FROM client_for_user WHERE id_cli = ?');
                                                    $get_debiteur->execute(array($row1['id_cl_cr']));
                                                    $fournis_info = $get_debiteur->fetch();

                                                    echo $fournis_info['prenom_cli'].' '.$fournis_info['nom_cli'];
                                                ?>  
                                                </td>
                                              <td class="text-right"><?php echo number_format($row1['reste_a_payer_cr'] * -1, 2, ',', ' '); ?></td>
                                              <td class="text-left">
                                                <?php
                                                    //on recupère le gestionnaire
                                                    $get_gestionnaire = $bdd->prepare('SELECT * FROM user WHERE id_cl = ?');
                                                    $get_gestionnaire->execute(array($row1['id_gest_cr']));
                                              ...
                                              <td>
                                                <a role="button" class="text-primary viewFacture" href="#" id="<?php echo $row1['id_cr']; ?>" option="<?php echo $row1['reste_a_payer_cr']; ?>" data-toggle="modal" data-target="#detail-facture">Consulter</a>
                                              </td>
                                            </tr>
                                          <?php 
                                            } 
                                    ?>
                                      ...
                                      <?php
                                        $sum_fact = $bdd->prepare("SELECT SUM(reste_a_payer_cr) AS montant_reste FROM note_credit WHERE id_UE_cr = ? AND reste_a_payer_cr != 0 ");
                  ...
                  <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                      <div class="modal-header badge-secondary text-white">
                        <div class="row">
                          <div class="col-md-10">
                            <h6 class="modal-title" id="exampleModalLabel">Détail dette note de crédit client <label class="text-secondary" id="idachat">0</label> <label class="text-secondary"  id="dette_initial">0</label></h6>
                          </div>
                          <div class="col-md-2">
          ...
          jQuery(document).ready(function($) 
          {
            ...
            function affiche_facture_apayer()
            {
              ...
              $.ajax(
                {
                  type  : 'POST',
                  url   : 'fonction/affiche_facture_apayer_client_note_credit.php',
            ...
            $('.viewFacture').click(function()
            {
              ...
              {
                type  : 'POST',
                url   : 'fonction/affiche_detail_dette_client_note_credit.php',
           ...
           $('#bootstrap-data-table').DataTable({
            ...
            $('#solder_facture').click(function()
            {
              ...
              if(Montpaye != '')
              {
                ...
                {
                  $('#Montpaye').addClass('is-invalid');
                  err3('Le montant à rembourser est invalide !');
                }
              }
              else
              {
                $('#Montpaye').addClass('is-invalid');
                err3('Veuillez saisir le montant à rembouser S.V.P !');
              }
            });
