    ...
    .container-tab1 {
      ...
      .form_card
       ...
       #libsearchcatart {
         ...
         <title>KEDIS Online! | Mes devis</title>
    <!--c head-->
    </head>
    <!--*****************************************************-->
    <body class="bg-light">
                ...
                <div class="card">
                  <h6 class="card-header">
                    <ul class="nav nav-pills card-header-pills">
                      ...
                      ?>
                        <li class="nav-item">
                          <a class="nav-link active" href="#"><strong>Mes offres/devis</strong></a>
                        </li>
                      <?php
                          }
                      ?>
                      ...
                      <div class="card-body">
                          <div class="card border-info mb-3" >
                              <div class="card-header text-left"><strong><b>Liste des offres/devis (Total : <?php echo $existallvente; ?>)</b></strong></div>
                                <div class="card-body">
                                    ...
                                    <div class="row">
                                      <div class="col-md-2">
                                        ...
                                        ?>
                                            ...
                                            <a role="button" class="btn btn-success btn-block" href="devis_init.php?id=<?php echo $getid;?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE;?>&nom_entreprise=<?php echo $nomE;?>&nom_unite_exploitation=<?php echo $nomUE;?>">
                                        ...
                                        ?>
                                          ...
                                          <div class="alert alert-primary" role="alert">
                                            Vous avez atteint le nombre maximal des offres/devis.
                                          </div>
                                  ...
                                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    ...
                                    <tbody class="table-bordered" id="table_article">
                                      <?php
                                       ...
                                       $view = $bdd->prepare("SELECT * FROM devis WHERE id_UE_dv = ? AND complete_devis != ? ORDER BY reference_dv DESC");
                                        $view->execute(array($idUE, 0));
                                        ...
                                        $existallvente = $view->rowCount();
                                         ...
                                         ?>   
                                            <tr>
                                              <td><a class="text-primary" href="viewDevis.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>&idoffre=<?php echo $row['id_dv']; ?>"><?php echo $row['num_dv']; ?></a></td>
                                              <td class="text-left"><?php echo $row['date_dv']; ?></td>
                                              <td class="text-left">
                                                <?php
                                                    $nbrcli = $bdd->prepare("SELECT * FROM client_for_user WHERE id_cli = ?"); 
                                                    $nbrcli->execute(array($row['id_cl_dv']));
                                              ...
                                              <td class="text-left"><?php echo $row['echeance_dv']; ?></td>
                                              <td class="text-right"><?php echo number_format($row['montant_facture'], 2, '.', ' '); ?></td>
                                              <td class="text-right"><?php echo number_format($row['paiemant_recu_dv'], 2, '.', ' '); ?></td>
                                              <td class="text-right"><?php echo number_format($row['reste_a_payer_dv'], 2, '.', ' '); ?></td>
                                              <td>
                                                <a class="text-primary" href="viewDevis.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>&idoffre=<?php echo $row['id_dv']; ?>">Détails</a>
                                              </td>
                                            </tr>
                                          <?php 
                                            } 
                                    ...
                                    <?php
                                        ...
                                        //somme total à payer
                                        $somme_mt_fact = $bdd->prepare("SELECT SUM(montant_facture) AS somme_ttc FROM devis WHERE   id_UE_dv = ? AND complete_devis = 1");
                                        $somme_mt_fact->execute(array($idUE));
                                        $fetch_ttc = $somme_mt_fact->fetch();
                                        $total_ttc = number_format($fetch_ttc['somme_ttc'], 2, '.', '');

                                        //somme montant payé
                                        $somme_paie = $bdd->prepare("SELECT SUM(paiemant_recu_dv) AS somme_paie FROM devis WHERE    id_UE_dv = ? AND complete_devis = 1");
                                        $somme_paie->execute(array($idUE));
                                        $fetch_paie = $somme_paie->fetch();
                                        $total_paie = number_format($fetch_paie['somme_paie'], 2, '.', '');

                                        //somme reste à payer
                                        $somme_reste = $bdd->prepare("SELECT SUM(reste_a_payer_dv) AS somme_reste FROM devis WHERE    id_UE_dv = ? AND complete_devis = 1");
             ...
             //affiche_devis();
              ...
              function affiche_devis()
              {
                ...
                var id_connexion = '<?php echo $id_connexion; ?>';
                 ...
                 $.ajax({
                  type : 'POST', 
                  url  : 'fonction/affiche_devis.php',
