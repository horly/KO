<?php
    ...
    $new_mess = $bdd->prepare('INSERT INTO message_admin(id_sender, message, datem, heurem, type, statut, dernier_message, code) VALUES(?, ?, ?, ?, ?, ?, ?, ?)');
	$new_mess->execute(array($getid, $message, $date, $heure, $type, $statut, 1, $key));


	$reqmess = $bdd->prepare('SELECT * FROM message_admin WHERE id_sender = ?  AND code = ?');
	$reqmess->execute(array($getid, $key));
	...
	$upadate2->execute(array($id_mess, $getid));
