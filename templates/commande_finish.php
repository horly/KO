...
<!--Début html-->
     ...
     .form_card
      ...
      #logoUE
      {
        height: 100px;
         ...
         <title>KEDIS Online! | Mes dépenses | commande</title>
    <!--c head-->
    </head>
    <!--*****************************************************-->
    <body class="bg-light">
    <!--Début body-->
              ...
              <?php 
                    ...
                    if(isset($_GET['id']) AND isset($_GET['id_connexion']) AND isset($_GET['idEU']) AND isset($_GET['nom_entreprise']) AND isset($_GET['nom_unite_exploitation']) AND isset($_GET['id_cmd']))  //si la variable id qu'on a transité existe dans l'url 
                    {
                      ...
                      if($id_connexion == $get_id_connexion)
                      {
                        ...
                        if($login_required == 1)
                        {
                          ...
                          $getid_cli = $bdd->prepare("SELECT * FROM commande WHERE id_cmd = ?");
                          $getid_cli->execute(array($id_cmd));
                          $fetch_id_four = $getid_cli->fetch();
                          $id_four = $fetch_id_four['id_four_cmd'];

                          //on récupère maintenant l'émail du client à partir de son id 
                          $get_email_four = $bdd->prepare("SELECT * FROM fournis_for_user WHERE id_four = ?");
                          $get_email_four->execute(array($id_four));
                          $fetch_email_four = $get_email_four->fetch();
                          $email_four = $fetch_email_four['email_four'];
                            ...
                            <div class="page-header float-right">
                                <div class="page-title">
                                    <ol class="breadcrumb text-right">
                                        <li><a href="commande.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>">Mes commandes</a></li>
                                        <li class="active">Aperçu commande</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card">
                  ...
                  <div class="card-body">
                    <div class="row">
                      ...
                      <div class="col-md-2">
                        <a role="button" class="btn btn-success btn-block" href="viewCommande.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>&id_cmd=<?php echo $id_cmd; ?>">
                          <span class="step size-21">
                                ...
                                <div class="card-body">
                                  ...
                                  <div class="input-group mb-3">
                                    ...
                                    <input type="text" class="form-control" placeholder="Modifier le nom du document" aria-label="Recipient's username" aria-describedby="basic-addon2" name="name-doc" id="name-doc" value="<?php
                                      if($infoart['titre_document_commande'] == '')
                                        {
                                          echo ucwords('bon de commande');
                                        }
                                        else
                                        {
                                            echo ucwords($infoart['titre_document_commande']);
                                        }
                                    ?>">
                                  </div>
                                  <label>Note de bas du document</label>
                                  <textarea class="form-control" rows="3" id="note-bas-page" placeholder="Modifier la note de bas de page"><?php echo $fetch_facture_modele['note_bas_commande']; ?></textarea>
                                </div>
          ...
          jQuery(document).ready(function($) 
          {
            ...
            $('.achat').addClass('active');
              ...
              function affiche_detail()
              {
                ...
                $.ajax({
                  type : 'POST', 
                  url  : 'fonction/detail_commande.php',
                  data : 'id_cmd=' + id_cmd + '&devise=' + devise + '&idUE=' + idUE +
                          '&nomE=' + nomE,
              ...
              $('#print_facture').click(function()
                {
                  ...
                  //window.open('facture/pdf/commande.php?id=' + getid + "&id_connexion=" + id_connexion + "&idEU=" + idUE + "&nom_entreprise=" + nomE + "&nom_unite_exploitation=" + nomUE + '&id_cmd=' + id_cmd + '&devise=' + devise, '_blank');

                  window.open('page_facture/facture_commande_pdf.php?id=' + getid + "&id_connexion=" + id_connexion + "&idEU=" + idUE + "&nom_entreprise=" + nomE + "&nom_unite_exploitation=" + nomUE + '&id_cmd=' + id_cmd + '&devise=' + devise, '_blank');
                });
             ...
             $('#save-name-doc').click(function()
              {
                ...
                $.ajax(
                  {
                    type  : 'POST',
                    url   : 'fonction/sava_info_doc_commande.php',
             ...
             $('#send-facture-mail').click(function()
              {
                  ...
                  var nomUE = '<?php echo $nomUE; ?>';
                  var nomE = '<?php echo $nomE; ?>';
                  var email_four = '<?php echo $email_four; ?>';

                  $.ajax(
                    {
                      type  : 'POST', 
                      url   : 'fonction/send_commande_mail.php',
                      data  : 'id_cmd=' + id_cmd + '&devise=' + devise + '&getid=' + getid +
                      ...
                      success:function(data)
                      {
                        ...
                        {
                          valid3("Document envoyé avec succès à l'adresse émail : " + email_four + ".");
                        }
                      }
                    });
              });
