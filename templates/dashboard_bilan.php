              ...
              <div class="card">
                <h6 class="card-header">
                  <nav class="nav nav-pills">
                    ...
                    <!--<a class="nav-link nav-item " href="#">Trésorerie</a>-->
                    <a class="nav-link nav-item active" href="#">Bilan Dettes et Stocks</a>
                ...
                <div class="card-body animated fadeIn">
                  <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th></th>
                            <th class="text-right">(<?php echo $devise; ?>)</th>
                        </tr>
                    </thead>
                    <tbody>
                      <?php

                          $gettotaldette = $bdd->prepare('SELECT SUM(montant_det) AS total_dette FROM dette_for_user WHERE id_UE_det = ?');
                          $gettotaldette->execute(array($idUE));
                          $fetch_som = $gettotaldette->fetch();

                          $total_dette = $fetch_som['total_dette'];
                        ?>
                        <tr>
                          <td><strong>Total Dettes à payer</td>
                          <td width="600" class="text-right"><?php echo number_format($total_dette, 2, '.', ' '); ?></td>
                        </tr>
                        <?php

                          $gettotalcreance = $bdd->prepare('SELECT SUM(montant_cre) AS total_creance FROM creance_for_user WHERE id_UE_cre = ?');
                          $gettotalcreance->execute(array($idUE));
                          $fetch_som_cre = $gettotalcreance->fetch();

                          $total_creance = $fetch_som_cre['total_creance'];
                        ?>
                        <tr>
                          <td><strong>Total Emprunt à encaisser</td>
                          <td width="600" class="text-right"><?php echo number_format($total_creance, 2, '.', ' '); ?></td>
                        </tr>

                        <?php

                          $gettotart = $bdd->prepare('SELECT * FROM article_for_user WHERE stock_actuel_art = 0 AND idUE_art = ?'); 
                          $gettotart->execute(array($idUE));

                          $countart = $gettotart->rowCount();
                        ?>
                        <tr>
                          <td><strong>Total Articles en rutpture de stock</td>
                          <td width="600" class="text-right"><?php echo number_format($countart, 2, '.', ' '); ?></td>
                        </tr>
                        <tr>
                          <td><strong></td>
                          <td width="600" class="text-right"><h6><?php echo number_format($total_dette + $total_creance + $countart, 2, '.', ' '); ?></h6></td>
                        </tr>
                    </tbody>
                  </table>
                </div>
