<?php
	...
	$verifVente = $bdd->prepare("SELECT * FROM note_credit WHERE id_UE_cr = ?");
    ...
    {
    	$selectmax = $bdd->prepare("SELECT MAX(reference_cr) AS maxref FROM note_credit WHERE id_UE_cr = ?");
        ...
        $num_credit = "N".str_pad($refCaise, 5, 0, STR_PAD_LEFT); //on crée un numéro de la note de crédit du genre N00005 exemple

	    $insertTempVent = $bdd->prepare("INSERT INTO note_credit(reference_cr, id_UE_cr, id_gest_cr, id_cl_cr, num_credit, complete_credit) VALUES(?, ?, ?, ?, ?, ?)");
	    $insertTempVent->execute(array($refCaise, $idUE, $getid, $idClient, $num_credit, $complete_note_credit));

	    header("location:facturation_note_credit.php?id=".$getid.'&id_connexion='.$id_connexion."&idEU=".$idUE."&nom_entreprise=".$nomE."&nom_unite_exploitation=".$nomUE."&refcaise=".$refCaise."&idClient=".$idClient);
