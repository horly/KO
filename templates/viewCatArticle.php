     ...
     .form_card
      ...
      .viewtva
      {
        display: none;
         ...
         <title>KEDIS Online! | Détails catégorie d'articles</title>
    <!--c head-->
    </head>
    <!--*****************************************************-->
    <body class="bg-light">
    <!--Début body-->
        ...
        <!--Code PHP-->
               <?php 
                    ...
                    if(isset($_GET['id']) AND isset($_GET['id_connexion']) AND isset($_GET['idEU']) AND isset($_GET['nom_entreprise']) AND isset($_GET['nom_unite_exploitation']) AND isset($_GET['idcatart']))  //si la variable id qu'on a transité existe dans l'url 
                    {
                      ...
                      if($id_connexion == $get_id_connexion)
                      {
                        ...
                        if($login_required == 1)
                        {
                          ...
                          $idcatart = $_GET['idcatart']; //on récupère l'id de l'article
                  ?>
         ...
         <div id="right-panel" class="right-panel">
          ...
          <?php include('navigation.php'); ?>
            ...
            <?php
              ...
              $getfService_cat = $bdd->prepare("SELECT * FROM categorie_article WHERE id_cat_art = ?");
              $getfService_cat->execute(array($idcatart)) or die (print_r($verifVente->errorInfo()));

              $fetctser = $getfService_cat->fetch();
                            ...
                            <div class="page-header float-right">
                                <div class="page-title">
                                    <ol class="breadcrumb text-right">
                                        <li><a href="stock.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>">Mes catégories d'articles</a></li>
                                        <li class="active">Détails catégorie d'articles</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
              ...
              <div class="animated fadeIn">
                <div class="card">
                  <div class="card-header">
                    <h6>Détails catégorie d'articles</h6>
                  </div>
                  <div class="card-body">
                    <div class="row">
                      <div class="col-md-2">
                ...
                <div class="modal-dialog" role="document">
                    ...
                    <div class="modal-content">
                      <div class="modal-header text-white bg-secondary">
                        <div class="row">
                          <div class="col-sm-10">
                            <h6 class="modal-title" id="myModalLabel">Modifier la catégorie d'articles</h6>
                          </div>
                          <div class="col-sm-2">
                      ...
                      <div class="modal-body">
                        ...
                        <div class="input-group mb-3">
                          ...
                          <input type="text" class="form-control" placeholder="Entrer le libellé de votre catégorie d'article" aria-label="Recipient's username" aria-describedby="basic-addon2" name="libelleser" id="libelleser" required="" value="<?php echo $fetctser['libelle_cat_art'];?>">
                        </div>
                      </div>
          ...
          jQuery(document).ready(function($) 
          {
            ...
            affiche_detail(); 
              ...
              function affiche_detail()
              {
                ...
                var idcatart = '<?php echo $idcatart; ?>';
                var idUE = '<?php echo $idUE; ?>';

                $.ajax({
                  type : 'POST', 
                  url  : 'fonction/detail_categorie_article.php',
                  data : 'idcatart=' + idcatart + '&idUE=' + idUE,
               ...
               //mis à jour du catégorie service
             $('#updatecatser').click(function()
                {
                  ...
                  if(libelleser != '')
                  {
                    $.ajax({
                      type : 'POST',
                      url : 'fonction/update_categorie_articles.php',
                      data : "libelleser=" + libelleser + '&idcatart=' + idcatart + 
                             "&idUE=" + idUE,
                      success:function(data)
                      {
                        //alert(data);
                        if(data == 1)
                        {
                          err3("Cette catégorie d'articles existe déjà !");
                          $('#libelleser').addClass('is-invalid');
                        }
                        else
                        {
                          $('#libelleser').removeClass('is-invalid');
                          valid3("Catégorie d'articles mis à jour avec succès !");
                  ...
                  {
                    $('#libelleser').addClass('is-invalid');
                    err3("Veuillez entrer le libellé du catégorie d'articles S.V.P!");
                  }
                });
                    ...
                    swal({
                            ...
                            function(){
                                ...
                                swal(
                                  {
                                    title: "Supprimé !!",
                                  text: "La catégorie d'arlticles a été supprimé avec succès !!",
                                  type: "success",
                                  confirmButtonColor: "#28a745",
                                  }, function(){ window.location = 'stock.php?id=' + getid + "&id_connexion=" + id_connexion + "&idEU=" + idUE + "&nom_entreprise=" + nomE + "&nom_unite_exploitation=" + nomUE; });
                            });
                  });
              ...
              function delete_cat_article()
              {
                ...
                $.ajax(
                  {
                    type  : 'POST',
                    url   : 'fonction/delete_cat_article.php',
                    data  : 'idcatart=' + idcatart + '&idUE=' + idUE, 
                    success:function(data)
                    {
