              ...
              <div class="card">
                <h6 class="card-header">
                  <nav class="nav nav-pills">
                    ...
                    <a class="nav-link nav-item active" href="#">Echéancier</a>
                  ...
                  <table class="table table-striped table-bordered">
                    ...
                    <tbody>
                      <?php
                        ...
                        $getsumfactcli = $bdd->prepare('SELECT SUM(reste_a_payer_fact) AS total_dette_client FROM vente_for_user WHERE id_UE_fact = ?');
                        $getsumfactcli->execute(array($idUE));

                        $fetch_tot = $getsumfactcli->fetch();

                        $total_dette_client = $fetch_tot['total_dette_client'];

                      ?>
                      <tr>
                        <td><strong>Mes clients me doivent</td>
                        <td class="text-right" width="400"><?php echo number_format($total_dette_client, 2, '.', ' '); ?></td>
                      </tr>
                      <?php

                        $getsumfactfour = $bdd->prepare('SELECT SUM(reste_a_payer_dp) AS total_dette_four FROM depense_for_user WHERE id_UE_dp = ?');
                        $getsumfactfour->execute(array($idUE));

                        $fetch_tot_four = $getsumfactfour->fetch();

                        $total_dette_four = $fetch_tot_four['total_dette_four'];

                      ?>
                      <tr>
                        <td><strong>Je dois à mes fournisseur</td>
                        <td class="text-right" width="400"><?php echo number_format($total_dette_four, 2, '.', ' '); ?></td>
                      </tr>
                      <tr>
                        <td><strong></td>
                        <td class="text-right" width="400"><h6><?php echo number_format($total_dette_client + $total_dette_four, 2, '.', ' '); ?></h6></td>
                      </tr>
                    </tbody>
                  </table>
                </div>
