    ...
    .container-tab1 {
      ...
      .form_card
       ...
       #libsearchcatart {
         ...
         <title>KEDIS Online! | Mes dépenses | Mes note de crédit/achat</title>
    <!--c head-->
    </head>
    <!--*****************************************************-->
    <body class="bg-light">
                            ...
                            <div class="page-header float-right">
                                <div class="page-title">
                                    <ol class="breadcrumb text-right">
                                        ...
                                        <li class="active">Mes note des crédit/achat</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                ...
                <div class="card">
                  <h6 class="card-header">
                     <ul class="nav nav-pills card-header-pills">
                      ...
                      ?>
                        <li class="nav-item">
                          <a class="nav-link active" href="#"><strong>Mes notes de crédit/achats</strong></a>
                        </li>
                       <?php
                          }
                      ?>
                      ...
                      ?>
                        <li class="nav-item">
                          <a class="nav-link" href="poste_depense.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>"><strong>Mes notes de frais</strong></a>
                        </li>
                      <?php
                          }
                      ?>
                      ...
                      <?php 
                          ...
                          $view = $bdd->prepare("SELECT * FROM note_credit_achat WHERE id_UE_cra = ? AND complete_credit != ?");
                                    ...
                                    <div class="row">
                                      <div class="col-md-3">
                                        ...
                                        ?>
                                           ...
                                           <a role="button" class="btn btn-success btn-block" href="note_credit_achat_init.php?id=<?php echo $getid;?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE;?>&nom_entreprise=<?php echo $nomE;?>&nom_unite_exploitation=<?php echo $nomUE;?>">
                                            <span class="step size-21">
                                                <i class="icon ion-document-text"></i>
                                            </span>
                                              &nbsp;Créer une note de crédit/achat</a>
                                        ...
                                        ?>
                                          ...
                                          <div class="alert alert-primary" role="alert">
                                            Vous avez atteint le nombre maximal de notes de crédit/achat.
                                          </div>
                                  ...
                                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                                    ...
                                    <tbody class="table-bordered" id="table_article">
                                      <?php
                                         ...
                                         $view = $bdd->prepare("SELECT * FROM note_credit_achat WHERE id_UE_cra = ? AND complete_credit != ? ORDER BY reference_cra DESC");
                                          $view->execute(array($idUE, 0));
                                        ...
                                        $existallvente = $view->rowCount();
                                         ...
                                         ?>   
                                           <tr>
                                              <td><a class="text-primary" href="viewNotecreditAchat.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>&id_note=<?php echo $row['id_cra']; ?>"><?php echo $row['num_credit']; ?></a></td>
                                              <td class="text-left"><?php echo $row['date_cra']; ?></td>
                                              <td class="text-left">
                                                <?php
                                                    $nbrcli = $bdd->prepare("SELECT * FROM fournis_for_user WHERE id_four = ?"); 
                                                    $nbrcli->execute(array($row['id_four_cra']));

                                                    $nameCli = $nbrcli->fetch();

                                                    echo $nameCli['societe_four']; //on affiche le nom du client 
                                                ?>
                                              </td>
                                              <td class="text-left"><?php echo $row['echeance_cra']; ?></td>
                                              <td class="text-right"><?php echo number_format($row['montant_facture'] * -1, 2, '.', ' '); ?></td>
                                              <td class="text-right"><?php echo number_format($row['paiemant_recu_cra'] * -1, 2, '.', ' '); ?></td>
                                              <td class="text-right"><?php echo number_format($row['reste_a_payer_cra'] * -1, 2, '.', ' '); ?></td>
                                              <td>
                                                <a class="text-primary" href="viewNotecreditAchat.php?id=<?php echo $getid; ?>&id_connexion=<?php echo $id_connexion; ?>&idEU=<?php echo $idUE; ?>&nom_entreprise=<?php echo $nomE; ?>&nom_unite_exploitation=<?php echo $nomUE; ?>&id_note=<?php echo $row['id_cra']; ?>">Détails</a>
                                              </td>                                            
                                            </tr>
                                          <?php 
                                            } 
                                    ...
                                    <?php
                                        ...
                                        //somme total à payer
                                        $somme_mt_fact = $bdd->prepare("SELECT SUM(montant_facture) AS somme_ttc FROM note_credit_achat WHERE id_UE_cra = ? AND complete_credit = 1");
                                        $somme_mt_fact->execute(array($idUE));
                                        $fetch_ttc = $somme_mt_fact->fetch();
                                        $total_ttc = number_format($fetch_ttc['somme_ttc'], 2, '.', '');

                                        //somme montant payé
                                        $somme_paie = $bdd->prepare("SELECT SUM(paiemant_recu_cra) AS somme_paie FROM note_credit_achat WHERE id_UE_cra = ? AND complete_credit = 1");
                                        $somme_paie->execute(array($idUE));
                                        $fetch_paie = $somme_paie->fetch();
                                        $total_paie = number_format($fetch_paie['somme_paie'], 2, '.', '');

                                        //somme reste à payer
                                        $somme_reste = $bdd->prepare("SELECT SUM(reste_a_payer_cra) AS somme_reste FROM note_credit_achat WHERE id_UE_cra = ? AND complete_credit = 1");
       ...
       <?php  include('chat_discution.php'); ?>
        ...
        <script type="text/javascript">
          jQuery(document).ready(function($) 
          {
            ...
            //affiche_note_credit_achat();

              function affiche_note_credit_achat()
              {
                ...
                var id_connexion = '<?php echo $id_connexion; ?>';
                 ...
                 $.ajax({
                  type : 'POST', 
                  url  : 'fonction/affiche_note_credit_achat.php',
